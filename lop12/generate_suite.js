...
// Unlock Web Audio on first mobile touch or click
    ...
    function initFontScale() {
      try {
        const saved = SafeStorage.getItem('VT12_FONT_SCALE_IDX');
    ...
    function changeFontScale(delta) {
      ...
      try {
        SafeStorage.setItem('VT12_FONT_SCALE_IDX', currentScaleIdx);
      } catch(e) {}
      applyFontScale();
    }
    ...
    function initTeacherName() {
      try {
        ...
        if (paramTeacher && paramTeacher.trim()) {
          ...
          const saved = SafeStorage.getItem('VT12_CUSTOM_TEACHER');
  ...
  <script>
    ...
    function renderDashboard() {
      ...
      chapters.forEach(ch => {
        ...
        for (let id = ch.start; id <= ch.end; id++) {
          ...
          const scoreStr = SafeStorage.getItem('VT12_LESSON_SCORE_' + id);
    ...
    function resetProgress() {
      if (confirm('Em có chắc chắn muốn xóa tất cả lịch sử điểm số của 25 bài học để làm lại từ đầu không?')) {
        for (let i = 1; i <= 25; i++) {
          SafeStorage.removeItem('VT12_LESSON_SCORE_' + i);
        }
        renderDashboard();
...
`;
  ...
  fs.writeFileSync('He_Thong_Trac_Nghiem_Vat_Li_12.html', html, 'utf8');
