  ...
  return `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'SF Pro Display', -apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    @media (orientation: portrait) and (max-width: 850px) {
      ...
      .graph-box svg { max-height: 340px !important; }
      .result-card {
        height: auto !important;
        overflow: visible !important;
      }
    ...
    const AudioEngine = {
      ...
      playCorrect() {
        try {
          ...
          chime.type = 'sine';
          chime.frequency.setValueAtTime(2093, now + 0.3);
      ...
      playWrong() {
        try {
          ...
          if (!this.ctx) return;
          const now = this.ctx.currentTime;
          // Encouraging downward slide melody (0.9s duration)
          const notes = [
            { fStart: 392.00, fEnd: 349.23, t: 0.00, dur: 0.24 }, // G4 -> F4
            { fStart: 311.13, fEnd: 293.66, t: 0.22, dur: 0.28 }, // Eb4 -> D4
            { fStart: 261.63, fEnd: 220.00, t: 0.48, dur: 0.42 }  // C4 -> A3
          ];
          ...
          notes.forEach(n => {
            ...
            osc.frequency.exponentialRampToValueAtTime(n.fEnd, now + n.t + n.dur);
      ...
      playWin() {
        try {
          ...
          const now = this.ctx.currentTime;
          const melody = [
            { freq: 392.00, t: 0.00, dur: 0.16 },
            { freq: 523.25, t: 0.14, dur: 0.18 },
            { freq: 659.25, t: 0.28, dur: 0.18 },
            { freq: 783.99, t: 0.42, dur: 0.35 },
            { freq: 659.25, t: 0.72, dur: 0.16 },
            { freq: 783.99, t: 0.86, dur: 0.42 }
          ];
          ...
          melody.forEach(n => {
            ...
            gain.gain.setValueAtTime(0.34, now + n.t);
            gain.gain.exponentialRampToValueAtTime(0.001, now + n.t + n.dur);
            osc.connect(gain);
            gain.connect(this.ctx.destination);
            osc.start(now + n.t);
          ...
          const chord = [341.63, 392.00, 659.25, 1046.50];
          chord.forEach((f, idx) => {
            const osc = this.ctx.createOscillator();
            const gain = this.ctx.createGain();
...
// Unlock Web Audio on first mobile touch or click
    ...
    function initFontScale() {
      try {
        const saved = SafeStorage.getItem('VT10_FONT_SCALE_IDX');
    ...
    function changeFontScale(delta) {
      ...
      try {
        SafeStorage.setItem('VT10_FONT_SCALE_IDX', currentScaleIdx);
      } catch(e) {}
      applyFontScale();
    }
      ...
      cloned.forEach(q => {
        if (q.type === 'mcq') {
          ...
          if (typeof ansIdx === 'string') {
            const letter = ansIdx.trim().toUpperCase();
            if (letter === 'A') ansIdx = 0;
            else if (letter === 'B') ansIdx = 1;
            else if (letter === 'C') ansIdx = 2;
            else if (letter === 'D') ansIdx = 3;
            else ansIdx = parseInt(ansIdx, 10);
          }
          if (isNaN(ansIdx) || ansIdx < 0 || ansIdx >= q.opts.length) ansIdx = 0;
          const originalCorrectOpt = q.opts[ansIdx];
          const items = q.opts.map((opt, i) => ({ text: opt, isCorrect: i === ansIdx }));
    ...
    function initTeacherName() {
      try {
        ...
        if (paramTeacher && paramTeacher.trim()) {
          ...
          SafeStorage.setItem('VT10_CUSTOM_TEACHER', teacherName);
        } else {
          const saved = SafeStorage.getItem('VT10_CUSTOM_TEACHER');
...
<head>
  ...
  <title>Hệ Thống Trắc Nghiệm Tương Tác Vật Lí 10 | Thầy Trần Mạnh Tùng</title>
...
<body>
      ...
      <div class="header-bar">
        ...
        <div class="header-group">
          ...
          <select class="lesson-select" id="lessonDropdown" onchange="changeLesson(this.value)">
            <optgroup label="CHƯƠNG I: MỞ ĐẦU">
              <option value="1">Bài 1: Khái quát về môn Vật lí</option>
              <option value="2">Bài 2: Vấn đề an toàn trong phòng thực hành</option>
              <option value="3">Bài 3: Thực hành tính sai số trong phép đo</option>
            </optgroup>
            <optgroup label="CHƯƠNG II: ĐỘNG HỌC">
              <option value="4">Bài 4: Độ dịch chuyển và quãng đường</option>
              <option value="5">Bài 5: Tốc độ và vận tốc</option>
              <option value="6">Bài 6: Thực hành: Đo tốc độ chuyển động</option>
              <option value="7">Bài 7: Đồ thị độ dịch chuyển – thời gian</option>
              <option value="8">Bài 8: Chuyển động biến đổi. Gia tốc</option>
              <option value="9">Bài 9: Chuyển động thẳng biến đổi đều</option>
              <option value="10">Bài 10: Sự rơi tự do</option>
              <option value="11">Bài 11: Thực hành: Đo gia tốc rơi tự do</option>
              <option value="12">Bài 12: Chuyển động ném</option>
            </optgroup>
            <optgroup label="CHƯƠNG III: ĐỘNG LỰC HỌC">
              <option value="13">Bài 13: Tổng hợp và phân tích lực</option>
              <option value="14">Bài 14: Định luật I Newton</option>
              <option value="15">Bài 15: Định luật II Newton</option>
              <option value="16">Bài 16: Định luật III Newton</option>
              <option value="17">Bài 17: Trọng lực và lực căng</option>
              <option value="18">Bài 18: Lực ma sát</option>
              <option value="19">Bài 19: Lực cản và lực nâng</option>
              <option value="20">Bài 20: Phương pháp giải bài toán động lực học</option>
              <option value="21">Bài 21: Moment lực. Cân bằng vật rắn</option>
              <option value="22">Bài 22: Thực hành: Hai lực song song</option>
            </optgroup>
            <optgroup label="CHƯƠNG IV: NĂNG LƯỢNG, CÔNG, CÔNG SUẤT">
              <option value="23">Bài 23: Năng lượng. Công cơ học</option>
              <option value="24">Bài 24: Công suất</option>
              <option value="25">Bài 25: Động năng, thế năng</option>
              <option value="26">Bài 26: Cơ năng và bảo toàn cơ năng</option>
              <option value="27">Bài 27: Hiệu suất</option>
            </optgroup>
            <optgroup label="CHƯƠNG V: ĐỘNG LƯỢNG">
              <option value="28">Bài 28: Động lượng</option>
              <option value="29">Bài 29: Định luật bảo toàn động lượng</option>
              <option value="30">Bài 30: Thực hành: Xác định động lượng</option>
            </optgroup>
            <optgroup label="CHƯƠNG VI: CHUYỂN ĐỘNG TRÒN">
              <option value="31">Bài 31: Động học của chuyển động tròn đều</option>
              <option value="32">Bài 32: Lực hướng tâm và gia tốc hướng tâm</option>
            </optgroup>
            <optgroup label="CHƯƠNG VII: BIẾN DẠNG CỦA VẬT RẮN. ÁP SUẤT CHẤT LỎNG">
              <option value="33">Bài 33: Biến dạng của vật rắn</option>
              <option value="34">Bài 34: Khối lượng riêng. Áp suất chất lỏng</option>
            </optgroup>
          </select>
  ...
  <script>
    ...
    function goToNextLesson() {
      if (currentLessonId < 34) {
        changeLesson(currentLessonId + 1);
      } else {
        alert('Chúc mừng em đã hoàn thành tất cả 34 bài học của toàn bộ 7 Chương Vật Lí 10!');
      }
    }
    ...
    function showResults() {
      ...
      try {
        const key = 'VT10_LESSON_SCORE_' + currentLessonId;
      ...
      if (totalPoints >= 9.0) {
        ...
        cmt.textContent = teacherName + ' biểu dương em! Em đã làm chủ hoàn hảo bài học Vật Lí 10 với tư duy sắc bén và năng lực vật lí vượt trội!';
    ...
    function renderDashboard() {
      const chapters = [
        { name: 'CHƯƠNG I: MỞ ĐẦU', start: 1, end: 3 },
        { name: 'CHƯƠNG II: ĐỘNG HỌC', start: 4, end: 12 },
        { name: 'CHƯƠNG III: ĐỘNG LỰC HỌC', start: 13, end: 22 },
        { name: 'CHƯƠNG IV: NĂNG LƯỢNG, CÔNG, CÔNG SUẤT', start: 23, end: 27 },
        { name: 'CHƯƠNG V: ĐỘNG LƯỢNG', start: 28, end: 30 },
        { name: 'CHƯƠNG VI: CHUYỂN ĐỘNG TRÒN', start: 31, end: 32 },
        { name: 'CHƯƠNG VII: BIẾN DẠNG CỦA VẬT RẮN. ÁP SUẤT CHẤT LỎNG', start: 33, end: 34 }
      ];
      ...
      chapters.forEach(ch => {
        ...
        for (let id = ch.start; id <= ch.end; id++) {
          ...
          const scoreStr = SafeStorage.getItem('VT10_LESSON_SCORE_' + id);
      ...
      document.getElementById('dashCompletedCount').textContent = completedCount + ' / 34';
    ...
    function resetProgress() {
      if (confirm('Em có chắc chắn muốn xóa tất cả lịch sử điểm số của 25 bài học để làm lại từ đầu không?')) {
        for (let i = 1; i <= 34; i++) {
          SafeStorage.removeItem('VT10_LESSON_SCORE_' + i);
        }
        renderDashboard();
...
`;
  fs.writeFileSync('He_Thong_Trac_Nghiem_Vat_Li_10_ThayTung.html', html, 'utf8');
  fs.writeFileSync('He_Thong_Trac_Nghiem_Vat_Li_10.html', html, 'utf8');
...
<head>
  ...
  <meta name="mobile-web-app-capable" content="yes" />
  <meta name="theme-color" content="#070a14" />
  <title>Vật Lí 10 - ${lessonTitle} | Thầy Trần Mạnh Tùng</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@500;600;700;800&display=swap" rel="stylesheet">
...
<body>
  ...
  <div class="container">
    <header>
      <div class="header-bar">
        ...
        <div class="header-group">
          <a href="./He_Thong_Trac_Nghiem_Vat_Li_10_ThayTung.html" class="nav-btn-hub">← Cổng Tổng Hợp</a>
          <div class="teacher-badge" id="teacherBadge" onclick="openTeacherModal()" title="Nhấp để thay đổi tên giáo viên">
            <span>👨‍🏫</span>
            <span id="teacherNameDisplay">Thầy Trần Mạnh Tùng</span>
            <span class="btn-edit-pen">✏️</span>
          ...
          <div class="lesson-title-badge">VẬT LÍ 10 - ${lessonTitle}</div>
        </div>
    ...
    <div class="result-card" id="resultCard">
      ...
      <div style="display: flex; justify-content: center; gap: 10px; flex-wrap: wrap;">
        <button class="btn-action" onclick="restartLesson()">Làm lại bài này ⟲</button>
        <a href="./He_Thong_Trac_Nghiem_Vat_Li_10_ThayTung.html" class="btn-action btn-next-step" style="text-decoration:none;">Về Cổng Tổng Hợp ➔</a>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function showResults() {
      ...
      try {
        const key = 'VT10_LESSON_SCORE_' + LESSON_ID;
        const oldScore = parseFloat(SafeStorage.getItem(key) || '0');
        if (totalPoints > oldScore) {
          SafeStorage.setItem(key, totalPoints.toFixed(2));
        }
...
console.log('2. Building 34 Standalone files...');
